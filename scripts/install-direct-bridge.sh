...
# Keep restrictive GE360 parent permissions while allowing the backend service
# to traverse /var/lib/ge360 and write Direct Bridge state/configuration.
STATE_PARENT="$(dirname "$STATE_DIR")"
setfacl -m "g:$GROUP_NAME:--x" "$STATE_PARENT"
setfacl -R -m "g:$GROUP_NAME:rwX" "$STATE_DIR" "$CONFIG_DIR"
setfacl -m "d:g:$GROUP_NAME:rwx" "$STATE_DIR" "$CONFIG_DIR"
