...
def validate_geometry(plan: NormalizedPlan, solved: SolverResult, rooms: list[RoomModel], *,
    ...
    bad = [x["wallId"] for x in wall_errors if not x["withinTolerance"]]
    if bad:
        errors.append(f"misure fuori tolleranza: {', '.join(bad)} (max {max_error:.1f} mm)")
    for d in solved.diagonal_meta:
        if not d["withinTolerance"]:
    ...
    return {
        ...
        "diagonals": solved.diagonal_meta, "suspects": solved.suspects,
