...
def normalize_payload(payload: PlanPayload, *, default_thickness_mm: float = 120,
    ...
    for wi, wall in enumerate(payload.walls):
        endpoints.append({"wall_index": wi, "wall_id": wall.id, "end": "a", "p": (wall.a.x, wall.a.y)})
        endpoints.append({"wall_index": wi, "wall_id": wall.id, "end": "b", "p": (wall.b.x, wall.b.y)})
    ...
    for i in range(len(endpoints)):
        for j in range(i + 1, len(endpoints)):
            if endpoints[i]["wall_index"] == endpoints[j]["wall_index"]:
                continue
            d_units = _distance(endpoints[i]["p"], endpoints[j]["p"])
            if d_units <= threshold_units:
                uf.union(i, j)
                merged_gaps_mm.append(d_units * scale)

    groups: dict[int, list[int]] = {}
    for i in range(len(endpoints)):
        groups.setdefault(uf.find(i), []).append(i)

    node_for_endpoint: dict[tuple[str, str], str] = {}
    nodes: dict[str, NormalizedNode] = {}
    for idx, members in enumerate(groups.values(), start=1):
        ...
        for i in members:
            ...
            source.append(key)
        nodes[node_id] = NormalizedNode(node_id, x, y, source)

    warnings: list[str] = []
    walls: list[NormalizedWall] = []
    wall_height_from_plan = payload.wallHeightM * 1000 if payload.wallHeightM > 0 else default_height_mm
    for wall in payload.walls:
        if wall.lengthCm is None:
            raise ValueError(f"wall {wall.id} has no authoritative lengthCm")
        start = node_for_endpoint[(wall.id, "a")]
        end = node_for_endpoint[(wall.id, "b")]
        if start == end:
            warnings.append(f"Wall {wall.id}: both endpoints collapsed into the same topology node")
        walls.append(
            NormalizedWall(
                id=wall.id,
                start_node=start,
                end_node=end,
                sketch_a=(wall.a.x, wall.a.y),
                sketch_b=(wall.b.x, wall.b.y),
                length_mm=wall.lengthCm * 10.0,
                source_length_cm=wall.lengthCm,
                thickness_mm=wall.thicknessMm or default_thickness_mm,
                height_mm=wall.heightMm or wall_height_from_plan,
                raw=wall.model_dump(mode="json"),
            )
        )
    ...
    return NormalizedPlan(
        ...
        rooms=[r.model_dump(mode="json") for r in payload.rooms],
        notes=list(payload.notes),
        scale_mm_per_unit=scale,
        merged_gaps_mm=merged_gaps_mm,
        warnings=warnings,
