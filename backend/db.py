...
class Database:
        ...
        with self.connect() as con:
            con.execute(
                ...
                ON CONFLICT(plan_id) DO UPDATE SET
                    name=excluded.name, updated_at=excluded.updated_at, status=excluded.status, path=excluded.path""",
                (plan_id, name, now, now, PlanStatus.RAW.value, 0, str(path)),
            )
            con.commit()

    def set_status(self, plan_id: str, status: PlanStatus, *, version: int | None = None,
                   quality: dict | None = None, needs_review: bool | None = None,
        ...
        if quality is not None:
            ...
            values.append(json.dumps(quality, ensure_ascii=False))
        ...
        with self.connect() as con:
            con.execute(f"UPDATE plans SET {', '.join(fields)} WHERE plan_id=?", values)
            con.commit()
    ...
    def get(self, plan_id: str) -> dict | None:
        ...
        quality_json = out.pop("quality_json")
        out["quality"] = json.loads(quality_json) if quality_json else None
        return out
